...
func (h *ActivityHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if v := q.Get("type"); v != "" { t := domain.ActivityType(v); filter.Type = &t }
	...
	if v := q.Get("deal_id"); v != "" { if id, err := uuid.Parse(v); err == nil { filter.DealID = &id } }
...
func (h *ActivityHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&a); err != nil { writeProblem(w, http.StatusBadRequest, "Bad Request", "invalid JSON body"); return }
	if err := a.Validate(); err != nil { writeProblem(w, http.StatusUnprocessableEntity, "Validation Error", err.Error()); return }
