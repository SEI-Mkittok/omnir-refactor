...
func (h *ContactHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if v := q.Get("stage"); v != "" { s := domain.ContactStage(v); filter.Stage = &s }
...
func (h *ContactHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&c); err != nil { writeProblem(w, http.StatusBadRequest, "Bad Request", "invalid JSON body"); return }
	if c.Stage == "" { c.Stage = domain.ContactStageLead }
	if err := c.Validate(); err != nil { handleDomainErr(w, err); return }
