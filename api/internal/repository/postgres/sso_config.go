...
func (r *ssoConfigRepo) Upsert(ctx context.Context, cfg *domain.SSOConfig) (*domain.SSOConfig, error) {
	...
	row := r.db.QueryRow(ctx, `
		INSERT INTO sso_configs
			(org_id, provider, client_id, client_secret, issuer_url, tenant_id, hd, attribute_mapping, enabled)
		...
		ON CONFLICT (org_id) DO UPDATE SET
			...
			updated_at        = now()
		RETURNING id, org_id, provider, client_id, client_secret, issuer_url, tenant_id, hd,
		          attribute_mapping, enabled, created_at, updated_at
	`, cfg.OrgID, cfg.Provider, cfg.ClientID, cfg.ClientSecret, cfg.IssuerURL,
		cfg.TenantID, cfg.Hd, mapping, cfg.Enabled)
	return scanSSOConfig(row)
}

func (r *ssoConfigRepo) GetByOrgID(ctx context.Context, orgID uuid.UUID) (*domain.SSOConfig, error) {
	row := r.db.QueryRow(ctx, `
		SELECT id, org_id, provider, client_id, client_secret, issuer_url, tenant_id, hd,
		       attribute_mapping, enabled, created_at, updated_at
...
func (r *ssoConfigRepo) GetByOrgSlug(ctx context.Context, slug string) (*domain.SSOConfig, error) {
	row := r.db.QueryRow(ctx, `
		SELECT s.id, s.org_id, s.provider, s.client_id, s.client_secret, s.issuer_url,
		       s.tenant_id, s.hd, s.attribute_mapping, s.enabled, s.created_at, s.updated_at
		FROM sso_configs s
...
func scanSSOConfig(row scannable) (*domain.SSOConfig, error) {
	...
	if err := row.Scan(&cfg.ID, &cfg.OrgID, &cfg.Provider, &cfg.ClientID, &cfg.ClientSecret,
		&cfg.IssuerURL, &cfg.TenantID, &cfg.Hd, &mappingJSON, &cfg.Enabled,
