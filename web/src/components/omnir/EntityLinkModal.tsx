...
export function EntityLinkModal<T>({
  ...
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && onClose()}>
      <DialogContent
        ref={contentRef}
        className="bottom-0 left-0 top-auto flex max-h-[calc(100dvh-5rem)] w-full translate-x-0 translate-y-0 flex-col gap-0 rounded-b-none rounded-t-2xl border p-0 shadow-xl data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom sm:left-[50%] sm:top-[50%] sm:grid sm:w-[calc(100%-2rem)] sm:max-w-sm sm:translate-x-[-50%] sm:translate-y-[-50%] sm:rounded-xl sm:data-[state=closed]:slide-out-to-left-1/2 sm:data-[state=closed]:slide-out-to-top-[48%] sm:data-[state=open]:slide-in-from-left-1/2 sm:data-[state=open]:slide-in-from-top-[48%]"
      ...
      >
        <DialogHeader
          className="border-b px-4 py-4 pr-14 sm:py-3"
        ...
        <div className="px-4 py-3">
          <div className="relative">
            ...
            <Input
              ...
              className="h-11 border pl-9 pr-9 text-base shadow-none sm:h-10 sm:text-sm"
              style={{
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {inlineError.actions.map((item) => (
                    <button
                      ...
                      type="button"
                      disabled={isPending}
                      className="min-h-11 rounded-md border border-amber-300 bg-white px-3 py-2 text-sm font-medium text-amber-900 transition-colors hover:bg-amber-100 disabled:opacity-50 sm:min-h-0 sm:px-2.5 sm:py-1.5 sm:text-xs"
                      onClick={async () => {
                        setIsPending(true)
                        try {
                          await item.action()
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-[calc(1rem+env(safe-area-inset-bottom,0px))] sm:max-h-64 sm:flex-none sm:pb-4">
          {statusMessage ? (
            <div
              id={listboxId}
              role="listbox"
              ...
              {results.map((item, index) => {
                ...
                return (
                  <button
                    ...
                    disabled={isPending}
                    className="min-h-11 w-full rounded-lg border px-3 py-3 text-left transition-colors disabled:opacity-50 sm:py-2"
                    style={{
